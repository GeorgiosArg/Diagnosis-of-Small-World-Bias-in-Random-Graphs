function f=FindRelativeFrequencyVector(adj)
% This function estimates the motifs' relative frequency vector of a network
% INPUT VARIABLES :
%  adj            : [matrix]      (nxn) adjacency matrix of a graph
%
% OUTPUT VARIABLES:
%  f              : [vector]      (1x6) the motifs' relative frequency vector

n=length(adj);                                    %number of vertices in A
As=adj|adj.';                                       %symmetric adjacency matrix
f=[0 0 0 0 0 0];
    ...
    for j=find(V1)
        ...
        for k=find(V2)
            ...
            for l=find(V3)
				...
				m=[  0      adj(i,j)  adj(i,k)  adj(i,l);
                   adj(j,i)     0     adj(j,k)  adj(j,l);
                   adj(k,i)   adj(k,j)    0     adj(k,l);
                   adj(l,i)   adj(l,j)  adj(l,k)    0   ];
